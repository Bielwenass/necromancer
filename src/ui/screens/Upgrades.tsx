...
import { BRANCHES, BranchColumn } from "./BranchColumn";
...
export function Upgrades({ onTabChange }: UpgradesProps) {
	...
	const [filter, setFilter] = useState<"all" | "affordable" | "purchased">(
		"all",
	);
	...
	return (
		<div className="necro">
			...
			<div className="upg-subnav">
				<button
					type="button"
					className={`upg-subnav-tab${view === "tree" ? " active" : ""}`}
				...
				<button
					type="button"
					className={`upg-subnav-tab${view === "workshop" ? " active" : ""}`}
					...
					<>
						...
						>
							<div>
								...
								>
									<span
										...
										style={{
											...
											lineHeight: 1,
											filter: "drop-shadow(0 0 6px rgba(212,168,87,0.25))",
										}}
									>
										{availablePoints}
									</span>
								...
								<div className="bar-meter" style={{ marginTop: 8, height: 4 }}>
									<i
										style={{
											width: `${totalEarned > 0 ? (spentPoints / totalEarned) * 100 : 0}%`,
							...
							<div>
								...
								<div style={{ marginTop: 10 }}>
									{BRANCHES.map((b, i) => {
										const branchNodes = UPGRADE_NODES.filter(
											(n) => n.branch === b.id,
										);
										const p = branchNodes.filter((n) =>
										...
										return (
											...
											>
												<div
													style={{ width: 8, height: 8, background: b.accent }}
												/>
												<span
													style={{
														fontSize: 12,
								...
								>
									{[
										...
										<button
											...
											className="flex w-full items-center gap-2 py-1 cursor-pointer"
										>
											<span
												className={`w-2.5 h-2.5 border border-[color:var(--rule-strong)] ${
													filter === o.v ? "bg-parchm" : "bg-transparent"
												}`}
											/>
											<span
												className={`text-[11px] ${
													filter === o.v ? "text-bone" : "text-muted"
												}`}
											>
												{o.l}
											</span>
						...
						<div
							style={{
								...
								overflow: "hidden",
								background:
									"radial-gradient(ellipse at 50% 50%, #110d08 0%, #0a0805 100%)",
							}}
						>
							{BRANCHES.map((b, i) => (
								<BranchColumn
