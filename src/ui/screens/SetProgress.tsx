	...
	return (
		<div className="h-[152px] bg-bg-panel border-t border-rule py-[14px] px-6 flex gap-4">
			<div className="w-[180px] flex flex-col justify-between">
				<div className="display text-xs text-parchm !tracking-[0.28em] uppercase">
					Set Progress
				</div>
				<div className="mono text-[10px] text-muted leading-[1.5]">
					Collect set pieces to
					<br />
					unlock cumulative perks.
					<br />
			...
			{SET_DEFS.map((s) => {
				const collected = s.pieces.filter((p) =>
					equippedBaseIds.includes(p),
				).length;
				return (
					<div
						key={s.id}
						className="flex-1 border border-rule px-[14px] py-[10px] flex gap-[14px] items-center bg-bg-panel-2"
					>
						...
						<div className="flex-1 min-w-0">
							<div className="display text-sm text-bone !tracking-[0.18em] uppercase">
								{s.name}
							</div>
								...
								{s.pieces.map((p) => {
									...
									return (
										<div
											...
											style={{
												background: has ? "var(--ink-parchm)" : "transparent",
												opacity: has ? 1 : 0.4,
											}}
										/>
									);
							...
							<div className="mt-2">
								{s.perks.map((p) => (
									...
									>
										<span
											className="mono text-[9px] min-w-[22px]"
											style={{
												color:
													collected >= p.at
														? "var(--c-coin)"
