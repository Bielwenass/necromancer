          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs text-bone whitespace-nowrap overflow-hidden text-ellipsis">{name}</div>
                <div className="mono text-[9px] tracking-[0.12em] uppercase mt-px" style={{ color: c }}>
                  {p.rarity} · {p.poolId}
                </div>
              </div>
            </div>
        ...
        <div className="flex gap-2">
          {(['common', 'uncommon', 'rare', 'epic', 'legendary'] as Rarity[]).map(r => (
            <div key={r} className="flex flex-col items-center flex-1">
              <div className="mono text-xs" style={{ color: rarityColor(r) }}>{sessionTotals[r]}</div>
              <div className="w-full h-0.5 mt-0.5 opacity-[0.6]" style={{ background: rarityColor(r) }} />
            </div>
          ))}
        </div>
      </div>
