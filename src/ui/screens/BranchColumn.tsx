...
export const BRANCHES: { id: string; name: string; epigraph: string; accent: string; nodePrefix: string }[] = [
  { id: 'summoning', name: 'Summoning', epigraph: 'Multiply the dead.',          accent: 'var(--c-bone)', nodePrefix: 's' },
  { id: 'command',   name: 'Command',   epigraph: 'Direct them with iron will.', accent: 'var(--c-coin)', nodePrefix: 'c' },
  { id: 'necromancy',name: 'Necromancy',epigraph: 'Wield death itself.',          accent: 'var(--c-soul)', nodePrefix: 'n' },
];

export function BranchColumn({
  branch, idx, purchased, filter, availablePoints, selectedNode, onSelectNode,
}: {
  ...
  const edges = branchNodes.flatMap(n =>
    n.unlocks.map(unlockId => {
      const child = UPGRADE_NODES.find(c => c.id === unlockId && c.branch === branch.id);
  ...
  const filteredNodes = branchNodes.filter(n => {
    ...
    if (filter === 'affordable') return state === 'unlocked' && availablePoints >= n.cost;
    return true;
  });
        ...
        {edges.map(([aId, bId], i) => {
          ...
          return (
            <line key={i} x1={na.x} y1={na.y} x2={nb.x} y2={nb.y}
              stroke={stroke} strokeWidth={w} strokeDasharray={dash} opacity={bothLocked ? 0.4 : 1} />
          );
        })}
        ...
        {branchNodes.map(n => {
          ...
          return (
            ...
            >
              ...
              {state === 'unlocked' && (
                <circle r={r + 4} fill="none" stroke={ringColor} strokeWidth="1" opacity="0.35" />
              )}
              {isSelected && (
                <>
                  <circle r={r + 6} fill="none" stroke="var(--c-coin)" strokeWidth="1" />
                  <circle r={r + 8} fill="none" stroke="var(--c-coin)" strokeWidth="0.5" opacity="0.5" />
                </>
              )}
              ...
              <g transform={`translate(${r - 2} ${-r + 2})`}>
                <rect x="-7" y="-6" width="14" height="12" fill="var(--bg-canvas)" stroke={ringColor} strokeWidth="0.5" />
                <text textAnchor="middle" y="3" fontSize="8" fontFamily="JetBrains Mono" fill={state === 'locked' ? 'var(--ink-dim)' : 'var(--ink-bone)'}>{n.tier}</text>
              </g>
              {state === 'locked' && !n.capstone && (
                <g transform={`translate(-15 18)`}>
                  <rect x="-4" y="-2" width="8" height="6" fill="var(--bg-canvas)" stroke="var(--ink-dim)" strokeWidth="0.6" />
                  <path d="M-2 -2 V-4 Q 0 -6 2 -4 V-2" fill="none" stroke="var(--ink-dim)" strokeWidth="0.6" />
                </g>
              )}
              <text textAnchor="middle" y={r + 22} fontSize="10" fontFamily="Spectral" fill={state === 'locked' ? 'var(--ink-dim)' : 'var(--ink-bone)'} letterSpacing="0.04em">
                {n.name}
              </text>
              <text textAnchor="middle" y={r + 34} fontSize="9" fontFamily="JetBrains Mono" fill={state === 'locked' ? 'var(--ink-faint)' : 'var(--ink-muted)'}>
                {state === 'purchased' ? '— OWNED —' : state === 'unlocked' ? `${n.cost} PTS` : `PREREQ REQ`}
              </text>
            </g>
          );
        })}
