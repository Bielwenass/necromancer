...
export function NodeDetail({
	...
	// Every node's branch is one of BRANCHES; bail rather than render half a panel.
	...
	return (
		<div className="w-[340px] border-l border-rule bg-bg-panel p-[22px] flex flex-col">
			...
			<div className="mt-4 flex items-center gap-[14px]">
				...
				<div>
					<div className="display text-xl text-bone !tracking-[0.12em]">
						{node.name}
					</div>
					<div
			...
			<div className="mt-[18px] p-[14px] border border-rule bg-bg-inset">
				<div className="text-xs text-bone leading-[1.5]">
					{node.description}
				</div>
				{node.flavor && (
			...
			{prereqNodes.length > 0 && (
				<div className="mt-[18px]">
					<div className="mono text-[9px] text-dim tracking-[0.16em]">
						REQUIRES
					</div>
					<div className="mt-[6px] flex flex-col gap-1">
						{prereqNodes.map((p) => (
							<div key={p.id} className="flex items-center gap-2">
								<span
									className="text-xs"
									style={{ color: p.met ? branch.accent : "var(--ink-dim)" }}
								>
									{p.met ? "✓" : "○"}
								</span>
								<span
									className="text-xs"
									style={{
										color: p.met ? "var(--ink-bone)" : "var(--ink-muted)",
									}}
								>
									{p.name}
								</span>
				...
				<div className="mt-[18px]">
					...
					<div className="mt-[6px] flex flex-wrap gap-[6px]">
						{unlockNodes.map((u) => (
							<div
								key={u.id}
								className="mono text-[10px] text-muted px-2 py-[3px] border border-rule-strong tracking-[0.08em]"
							>
								{u.name}
							</div>
						))}
			...
			<div className="mt-auto pt-[18px]">
				<div className="flex justify-between items-baseline mb-2">
					...
					<span>
						...
						<span className="mono text-[10px] text-muted tracking-[0.12em]">
							POINTS
						</span>
					</span>
				</div>
					...
					<button
						...
						style={{
							...
							background: canPurchase
								? "linear-gradient(180deg, rgba(232,220,192,0.05), transparent 80%)"
								: "transparent",
							color: canPurchase ? branch.accent : "var(--ink-dim)",
							cursor: canPurchase ? "pointer" : "not-allowed",
						}}
					>
						{!prereqNodes.every((p) => p.met)
							? "LOCKED"
