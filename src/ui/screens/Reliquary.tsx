			...
			<div className="stage flex-col">
				...
				<div className="h-10 flex items-center border-b border-rule px-6 bg-bg-panel shrink-0">
					<div className="flex gap-[22px]">
						{(["All", "By Slot", "Sets", "New"] as const).map((label, i) => (
							...
							>
								<span
									className={`display !tracking-[0.22em] uppercase text-[12px] ${i === 0 ? "text-bone" : "text-muted"}`}
								>
									{label}
								</span>
								<span
									className={`mono text-[9px] ${label === "New" ? "text-coin" : "text-dim"}`}
								>
									{label === "All"
										? inventory.length
										: label === "New"
				...
				<div className="flex-1 flex min-h-0">
					...
					<div
						...
						style={{
							background: "linear-gradient(180deg, #15110b 0%, #0f0c08 100%)",
						}}
					>
						{selectedRelic ? (
							<RelicDetail
