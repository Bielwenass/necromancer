...
export function importSave(json: string): ImportResult {
  try {
    ...
    if (parsed.version !== SAVE_VERSION) {
      return { ok: false, error: `Version mismatch (expected ${SAVE_VERSION}, got ${parsed.version ?? 'unknown'}).` };
    }
    const required = ['resources', 'workshop', 'units', 'squads', 'dungeons', 'relics', 'upgrades', 'surge', 'gacha', 'meta'];
