...
import type { GameState, Squad, Resources } from './types';
...
function deriveFightSeed(squadId: string, dungeonId: string, clearCount: number): number {
...
function isLossless(input: Record<string, number>, output: Record<string, number>): boolean {
...
function computeTravelTime(state: GameState, dungeonId: string): number {
  ...
  // Surge ignored: surgeSpeedMultiplier treated as 1.
...
function generateLootSeeded(
  ...
  const bones = Math.round((lt.bonesMin + rand() * (lt.bonesMax - lt.bonesMin)) * clearBonus * surgeYield);
  const coins = Math.round((lt.coinsMin + rand() * (lt.coinsMax - lt.coinsMin)) * clearBonus * surgeYield);
  const corpses = Math.round((lt.corpseMin + rand() * (lt.corpseMax - lt.corpseMin)) * clearBonus);
  const souls = rand() < lt.soulChance * surgeYield ? 1 : 0;
...
function initialEvent(
  ...
  switch (squad.state) {
    ...
    case 'traveling': {
      ...
      return { kind: 'outboundArrive', ticksUntil: cursor + remaining, squadId: squad.id };
    }
    ...
    case 'returning': {
      ...
      squad._phaseEnd = cursor + remaining;
      return { kind: 'returnArrive', ticksUntil: cursor + remaining, squadId: squad.id };
    }
...
function processEvent(
  ...
  const squad = state.squads.find(s => s.id === event.squadId) as WorkingSquad | undefined;
  if (!squad || !squad.targetDungeonId) return null;
  ...
  switch (event.kind) {
    case 'outboundArrive': {
      ...
      if (!dungeon.unlocked) {
        ...
        return { kind: 'returnArrive', ticksUntil: cursor + tt, squadId: squad.id };
      }
      ...
      return { kind: 'fightDone', ticksUntil: cursor + outcome.durationTicks, squadId: squad.id };
    }
    ...
    case 'returnArrive': {
      ...
      if (state.derived.autoDeploy && !wasManualRecall) {
        ...
        if (dn && dn.unlocked && totalUnits(squad.composition) > 0) {
          squad.state = 'traveling';
          squad.position = 0;
          const tt = computeTravelTime(state, squad.targetDungeonId);
          squad._phaseStart = cursor;
          squad._phaseEnd = cursor + tt;
          return { kind: 'outboundArrive', ticksUntil: cursor + tt, squadId: squad.id };
        }
      }
...
function finalizeSquads(state: GameState, cursor: number): void {
  for (const s of state.squads as WorkingSquad[]) {
    if (s._phaseStart !== undefined && s._phaseEnd !== undefined && s._phaseEnd > cursor) {
