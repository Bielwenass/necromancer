...
export function tickTierA(
  ...
  let cAx = 0, cAy = 0, cAc = 0;
  let cBx = 0, cBy = 0, cBc = 0;
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (u.side === 'a') {
      aSumX[ci] += u.x; aSumY[ci] += u.y;
      aSumVx[ci] += u.vx; aSumVy[ci] += u.vy; aCount[ci]++;
      cAx += u.x; cAy += u.y; cAc++;
    } else {
      bSumX[ci] += u.x; bSumY[ci] += u.y;
      bSumVx[ci] += u.vx; bSumVy[ci] += u.vy; bCount[ci]++;
      cBx += u.x; cBy += u.y; cBc++;
    }
  }
  ...
  for (let i = 0; i < N; i++) {
    const u = units[i];
    const ux = u.x, uy = u.y, uid = u.id, uvx = u.vx, uvy = u.vy;
    ...
    let sSumX = 0, sSumY = 0, sSumVx = 0, sSumVy = 0, sCount = 0;
    let eSumX = 0, eSumY = 0, eCount = 0;
    ...
    for (let gy = y0; gy <= y1; gy++) {
      ...
      for (let gx = x0; gx <= x1; gx++) {
        const gi = rowBase + gx;
        if (sameIsA) {
          sSumX += aSumX[gi]; sSumY += aSumY[gi];
          sSumVx += aSumVx[gi]; sSumVy += aSumVy[gi]; sCount += aCount[gi];
          eSumX += bSumX[gi]; eSumY += bSumY[gi]; eCount += bCount[gi];
        } else {
          sSumX += bSumX[gi]; sSumY += bSumY[gi];
          sSumVx += bSumVx[gi]; sSumVy += bSumVy[gi]; sCount += bCount[gi];
          eSumX += aSumX[gi]; eSumY += aSumY[gi]; eCount += aCount[gi];
        }
      }
    }
    ...
    let seekTx, seekTy, haveSeek = false;
    if (eCount > 0) {
      const inv = 1 / eCount;
      seekTx = eSumX * inv; seekTy = eSumY * inv; haveSeek = true;
    } else {
      const gc = sameIsA
        ? (cBc > 0 ? { x: cBx / cBc, y: cBy / cBc } : null)
        : (cAc > 0 ? { x: cAx / cAc, y: cAy / cAc } : null);
      if (gc) { seekTx = gc.x; seekTy = gc.y; haveSeek = true; }
    }
    if (haveSeek) {
      const dx = seekTx! - ux;
      const dy = seekTy! - uy;
    ...
    if (stats) {
      ...
      stats.queryCalls++;
      if (neighbors.length > stats.maxNeighbors) stats.maxNeighbors = neighbors.length;
    }
